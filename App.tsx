...
export default function App() {
  // User State (Persisted in sessionStorage for refresh)
  const [user, setUser] = useState<User | null>(() => {
    try {
      const saved = sessionStorage.getItem('busted_user');
  ...
  useEffect(() => {
    if (user) sessionStorage.setItem('busted_user', JSON.stringify(user));
    else sessionStorage.removeItem('busted_user');
  }, [user]);

  // View State (Synced with Hash for back/forward/refresh support)
  const [currentView, _setCurrentView] = useState<ViewState>(() => {
    const hash = window.location.hash.replace('#', '');
    return (hash as ViewState) || 'home';
  });

  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '');
      _setCurrentView((hash as ViewState) || 'home');
    };
        ...
        .then((data: any[]) => {
          if (data && data.length > 0) {
            const mappedActivity: ActivityItem[] = data.map(item => ({
              ...
              status: item.status === 'draft_redirected' ? 'Pending' : 'Pending',
            ...
            <div className="w-full relative animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {currentView === 'dashboard' && (
                <div className="w-full px-6 md:px-12 pb-20 relative z-10">
                  ...
                  {/* Stats Strip - Only for logged-in users */}
                  {!user?.isGuest && (
                  ...
                  {/* Bottom Split: Recent Activity + Local News Feed */}
                  {!user?.isGuest && (
                    <div className="grid grid-cols-1 xl:grid-cols-1 gap-16 xl:gap-24 relative z-10">
                          ...
                          {activity.slice(0, 3).map((item) => (
                            <div key={item.id} className="flex flex-col md:flex-row md:items-center justify-between py-4 px-4 -mx-4 hover:bg-white/[0.03] rounded-xl transition-colors group cursor-default">
                              ...
                              <div className="flex items-center gap-6 pl-12 md:pl-0">
                                <div className={`text-xs px-2.5 py-1 rounded-md font-bold tracking-wide ${item.status === 'Verified' ? 'bg-green-500/10 text-green-400' :
                      ...
                      {/* Activity section spans full width now */}
